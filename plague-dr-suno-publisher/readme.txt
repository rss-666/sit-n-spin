...
Soundtrack mode synchronizes title, description, excerpt, status, album, duration, genre, buy/stream URL, and cover artwork. Video mode synchronizes title, description, status, duration, runtime note, YouTube/Vimeo/direct-video URL, and artwork. A dedicated Artist / Featured Artist name is highlighted in amber, while centered lyrics are stored once and rendered on both linked pages without entering archive excerpts. Updating a release updates the same linked native records instead of creating duplicates.

== Installation ==

1. Upload `plague-dr-suno-publisher-1.4.1.zip` through Plugins -> Add New Plugin -> Upload Plugin.
...
= 1.4.1 =
* Added a vertically scrollable lyrics panel beneath the homepage/media Suno popout player.
* Added highlighted artist credit and a direct full-Soundtrack link to the popout.
* Preserved verse/chorus line breaks and keyboard access without pretending lyrics are time-synchronized.
* Made the hosted-player dialog responsive and nearly full-screen on mobile.

= 1.4.0 =
* Added a dedicated Artist / Featured Artist credit separate from descriptions.
* Styled artist credits in the theme amber accent for stronger visual distinction.
* Centered lyrics headings and body copy in a narrower reading column on native and generic pages.
