...
final class PDRS_PDU_Integration {
    ...
    public static function mode_for( int $song_post_id ): string {
        ...
        return self::MODE === $mode ? self::MODE : 'destination';
    }
    ...
    public static function sync_song( int $song_post_id ) {
        ...
        if ( self::MODE !== self::mode_for( $song_post_id ) ) {
        ...
        if ( ! PDRS_Suno_URL::valid_song_id( $suno_id ) ) {
            return new WP_Error( 'pdrs_song_id_missing', __( 'Save a valid Suno song URL before syncing the theme Soundtrack.', 'plague-dr-suno-publisher' ) );
        }
        ...
        update_post_meta( $track_id, '_pdrs_suno_id', $suno_id );
        ...
        self::sync_url_meta( $track_id, 'pdu_audio_url', self::audio_url( (string) get_post_meta( $song_post_id, '_pdrs_audio_url', true ) ) );
    ...
    public function sync_after_save( int $post_id, WP_Post $post ): void {
        ...
        if ( ! get_post_meta( $post_id, '_pdrs_song_id', true ) ) {
            return; // Quick Add writes relationship metadata immediately after wp_insert_post().
        }
    ...
    public function trash_linked_track( int $post_id ): void {
        ...
        $linked_id = self::linked_track_id( $post_id );
        if ( $linked_id && 'trash' !== get_post_status( $linked_id ) ) {
    ...
    public function single_track_player( string $content ): string {
        if ( is_admin() || is_feed() || ! is_singular( self::THEME_POST_TYPE ) || get_the_ID() !== get_queried_object_id() ) {
            return $content;
        }
    ...
    public function frontend_assets(): void {
        ...
        $songs = get_posts(
            array(
                ...
                'post_status'    => 'publish',
                'posts_per_page' => 100,
                'meta_key'       => '_pdrs_placement_mode',
                'meta_value'     => self::MODE,
                'no_found_rows'  => true,
            )
        );
        $tracks = array();
